'''
*********************************************************************************************
  *File: IMU_Reading_Recording_Plotting.py
  *Project: IMU_python_reading_3.0
  *Filepath: /home/guoyucan/Downloads/IMU_Python_Reading/IMU_python_reading_3.0/IMU_Reading_Recording_Plotting.py 
  *File Created: Friday, 24th September 2021 10:13:44 pm
  *Author: Guo Yucan, 12032421@mail.sustech.edu.cn 
  *Last Modified: Friday, 24th September 2021 10:13:46 pm
...
def main():
    ...
    time_sequence  = 0
    label = ["acceleration around x-axis","acceleration around yaxis","acceleration around z-axis","roll","pitch","yaw"]
    color_line = ['r--','g--','b--','c-','y-','m-']

    fig = plt.figure(figsize=(16,10))

    plot = [None]*2
    plot[0] = plt.subplot(121)
    plot[1] = plt.subplot(122)


    for i in range(0,3):
        plot[0].plot(time_sequence, imu_data[0][i], color_line[i], label[i])
        plot[0].set_xlabel("time sequence")
        plot[0].set_ylabel(label[i])
        # plot[0].legend()
        plot[0].grid()

        # [subfig_1].xlabel("time sequence")
        # subfig_1.ylabel("accelerations (g)")
    
    for i in range(3,6):
        plot[1].plot(time_sequence, imu_data[0][i], color_line[i], label[i])
        plot[1].set_xlabel("time sequence")
        plot[1].set_ylabel(label[i])
        # plot[1].legend()
        plot[1].grid()
    ...
    while True:
        ...
        ## Update the plot with new data accessed
